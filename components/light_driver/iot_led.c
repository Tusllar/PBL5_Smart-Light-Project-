...
typedef struct {
    ...
    // --- Thêm các trường để lưu thông tin timer ---
    uint32_t timer_freq_hz;       // tần số timer
    uint32_t timer_duty_resolution; // độ phân giải duty
} iot_light_t;
...
static IRAM_ATTR esp_err_t _iot_set_fade_with_time(
    ledc_mode_t speed_mode,
...
{
    if (!g_light_config) {
        ESP_LOGE("iot_led", "g_light_config chưa khởi tạo!");
    ...
    // Lấy thông tin timer từ cấu hình đã lưu
    uint32_t timer_source_clk = g_light_config->timer_freq_hz;
    uint32_t duty_resolution = g_light_config->timer_duty_resolution;
    uint32_t precision = (0x1U << duty_resolution);
    uint32_t freq = timer_source_clk;

    if (duty_delta == 0) {
        return _iot_set_fade_with_step(speed_mode, channel, target_duty, 0, 0);
    }

    // Tính tổng số cycle cho fade
...
uint32_t intr_status = TIMERG0.int_st_timers.val; // thay cho int_st.val
